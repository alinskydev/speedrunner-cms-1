		...
		for ($i=0, $s = count($_FILES['upload']['name']); $i < $s; $i++) {
			if (!empty($_FILES['upload']['name'][$i])) {
				...
				if ($_FILES['upload']['error'][$i] > 0) {
                    ...
                    $name = \yii\helpers\BaseInflector::transliterate(basename($_FILES['upload']['name'][$i]));
                    ...
                    for($n = 1; file_exists($file);$n++){
                        $file = $dir.DIRECTORY_SEPARATOR."{$path_parts['filename']}_copy_$n.{$path_parts['extension']}";
                    };
					if (!@move_uploaded_file($_FILES['upload']['tmp_name'][$i], $file)) {
						$this->_errorData($name, 'Unable to save uploaded file');
					} else {
